...
async function request<T = unknown>(url: string, options: RequestOptions = {}): Promise<T> {
  ...
  try {
    ...
    if (/network|fetch/i.test(msg))
      throw new OrdPawApiError('网络错误：无法连接到服务器', 0, 'network');
...
export class API {
  ...
  async getAgent(id: string): Promise<Agent> {
    return this.cachedRequest(
      `agent:${id}`,
      () => request<Agent>(`${this.baseUrl}/agents/${id}`),
      30_000
    );
  }

  async updateAgent(id: string, data: Partial<Agent>): Promise<Agent> {
    const result = await request<Agent>(`${this.baseUrl}/agents/${id}`, {
      method: 'PUT',
      body: data,
    });
    this.cache.invalidate('agents');
  ...
  async createConversation(agentId: string, title?: string): Promise<Conversation> {
    const result = await request<Conversation>(`${this.baseUrl}/conversations`, {
      method: 'POST',
      body: { agentId, title },
    });
  ...
  async getConversation(id: string): Promise<Conversation> {
    return this.cachedRequest(
      `conversation:${id}`,
      () => request<Conversation>(`${this.baseUrl}/conversations/${id}`),
      30_000
    );
  }

  async deleteConversation(id: string): Promise<void> {
    await request<void>(`${this.baseUrl}/conversations/${id}`, {
      method: 'DELETE',
      expectJson: false,
    });
    this.cache.invalidatePattern('conversations');
  ...
  async sendMessage(conversationId: string, content: string): Promise<unknown> {
    const result = await request(`${this.baseUrl}/chat`, {
      method: 'POST',
  ...
  async getPrompts(): Promise<PromptTemplate[]> {
    return this.cachedRequest(
      'prompts',
      () => request<PromptTemplate[]>(`${this.baseUrl}/prompts`),
      30_000
    );
  }
  async createPrompt(data: Partial<PromptTemplate>): Promise<PromptTemplate> {
    const result = await request<PromptTemplate>(`${this.baseUrl}/prompts`, {
      method: 'POST',
      body: data,
    });
  ...
  async updatePrompt(id: string, data: Partial<PromptTemplate>): Promise<PromptTemplate> {
    const result = await request<PromptTemplate>(`${this.baseUrl}/prompts/${id}`, {
      method: 'PUT',
      body: data,
    });
    this.cache.invalidate('prompts');
  ...
  async getPlugins(): Promise<PluginInstance[]> {
    return this.cachedRequest(
      'plugins',
      () => request<PluginInstance[]>(`${this.baseUrl}/plugins`),
      30_000
    );
  }
  async installPlugin(data: unknown): Promise<PluginInstance> {
    const result = await request<PluginInstance>(`${this.baseUrl}/plugins/install`, {
      method: 'POST',
      body: data,
    });
  ...
  async updatePluginConfig(id: string, config: Record<string, unknown>): Promise<void> {
    await request<void>(`${this.baseUrl}/plugins/${id}/config`, {
      method: 'PUT',
      body: { config },
      expectJson: false,
    });
  ...
  async getSettings(): Promise<Settings> {
    return this.cachedRequest(
      'settings',
      () => request<Settings>(`${this.baseUrl}/settings`),
      60_000
    );
  }
  async updateSettings(data: Partial<Settings>): Promise<void> {
    await request<void>(`${this.baseUrl}/settings`, {
      method: 'PUT',
      body: data,
      expectJson: false,
    });
  ...
  async getStats(): Promise<StatsResponse> {
    return request<StatsResponse>(`${this.baseUrl}/stats`);
  }
  ...
  async executeSkill(id: string, params?: Record<string, unknown>): Promise<SkillExecuteResult> {
    return request<SkillExecuteResult>(`${this.baseUrl}/skills/${id}/execute`, {
      method: 'POST',
      body: { params: params || {} },
    });
  }
  ...
  async getMcpServers(): Promise<McpServer[]> {
    return request<McpServer[]>(`${this.baseUrl}/mcp`);
  }
  ...
  async getScripts(): Promise<Script[]> {
    return this.cachedRequest(
      'scripts',
      () => request<Script[]>(`${this.baseUrl}/scripts`),
      30_000
    );
  }
  async getScript(id: string): Promise<Script> {
    return this.cachedRequest(
      `script:${id}`,
      () => request<Script>(`${this.baseUrl}/scripts/${id}`),
      30_000
    );
  }
  ...
  async updateScript(id: string, data: Partial<Script>): Promise<Script> {
    const result = await request<Script>(`${this.baseUrl}/scripts/${id}`, {
      method: 'PUT',
      body: data,
    });
    this.cache.invalidate('scripts');
  ...
  async executeScript(
    ...
    return request<ScriptExecutionResult>(`${this.baseUrl}/scripts/${id}/execute`, {
      method: 'POST',
      body: { args, context },
    });
  }
  async useScript(
    ...
    return request<ScriptExecutionResult>(`${this.baseUrl}/scripts/use`, {
      method: 'POST',
      body: { name, args, context },
    });
  }
  ...
  async getProviders(): Promise<Provider[]> {
    return this.cachedRequest(
      'providers',
      () => request<Provider[]>(`${this.baseUrl}/providers`),
      30_000
    );
  }
  async getProviderModels(providerId: string): Promise<ModelInfo[]> {
    return this.cachedRequest(
      ...
      () => request<ModelInfo[]>(`${this.baseUrl}/providers/${providerId}/models`),
      30_000
    );
  }
  async createProvider(data: Partial<Provider>): Promise<Provider> {
    const result = await request<Provider>(`${this.baseUrl}/providers`, {
      method: 'POST',
      body: data,
    });
  ...
  async updateProvider(id: string, data: Partial<Provider>): Promise<Provider> {
    const result = await request<Provider>(`${this.baseUrl}/providers/${id}`, {
      method: 'PUT',
      body: data,
    });
    this.cache.invalidate('providers');
  ...
  async getTestSuite(id: string): Promise<TestSuite> {
    return this.cachedRequest(
      `test-suite:${id}`,
      () => request<TestSuite>(`${this.baseUrl}/test-suites/${id}`),
      30_000
    );
  }
  async createTestSuite(data: {
    ...
    const result = await request<TestSuite>(`${this.baseUrl}/test-suites`, {
      method: 'POST',
      body: data,
    });
  ...
  async updateTestSuite(id: string, data: Partial<TestSuite>): Promise<TestSuite> {
    const result = await request<TestSuite>(`${this.baseUrl}/test-suites/${id}`, {
      method: 'PUT',
      body: data,
    });
    this.cache.invalidatePattern('test-suites');
  ...
  async deleteTestSuite(id: string): Promise<void> {
    await request<void>(`${this.baseUrl}/test-suites/${id}`, {
      method: 'DELETE',
      expectJson: false,
    });
    this.cache.invalidatePattern('test-suites');
  ...
  async runTestSuite(id: string): Promise<TestRun> {
    const result = await request<TestRun>(`${this.baseUrl}/test-suites/${id}/run`, {
      method: 'POST',
    });
    this.cache.invalidate(`test-suite:${id}`);
    return result;
  ...
  async getTestRuns(id: string): Promise<TestRun[]> {
    return this.cachedRequest(
      `test-runs:${id}`,
      () => request<TestRun[]>(`${this.baseUrl}/test-suites/${id}/runs`),
      30_000
    );
  }
  async createTestCase(suiteId: string, data: Partial<TestCase>): Promise<TestCase> {
    const result = await request<TestCase>(`${this.baseUrl}/test-suites/${suiteId}/cases`, {
      method: 'POST',
      body: data,
    });
  ...
  async updateTestCase(id: string, data: Partial<TestCase>): Promise<TestCase> {
    const result = await request<TestCase>(`${this.baseUrl}/test-cases/${id}`, {
      method: 'PUT',
      body: data,
    });
  ...
  async deleteTestCase(id: string): Promise<void> {
    await request<void>(`${this.baseUrl}/test-cases/${id}`, {
      method: 'DELETE',
      expectJson: false,
    });
    this.cache.invalidatePattern('test-');
  ...
  subscribeDebugStream(
    ...
    if (onLog) es.addEventListener('log', (e: MessageEvent) => onLog(JSON.parse(e.data)));
    if (onEvent) es.addEventListener('event', (e: MessageEvent) => onEvent(JSON.parse(e.data)));
    return es;
  }
  ...
  async getComponentManifest(): Promise<{ version: string; items: ComponentContribution[] }> {
    return this.cachedRequest(
      'components',
      () =>
        request<{ version: string; items: ComponentContribution[] }>(
          `${this.baseUrl}/components/manifest`
        ),
      60_000
    );
  }
  async getComponentTree(): Promise<ComponentTreeResponse> {
    return this.cachedRequest(
      'component-tree',
      () => request<ComponentTreeResponse>(`${this.baseUrl}/components/tree`),
      60_000
    );
  }
  async getComponentRelationships(): Promise<{
    relationships: Array<{ from: string; to: string }>;
  }> {
    return this.cachedRequest(
      'component-relationships',
      () =>
        request<{ relationships: Array<{ from: string; to: string }> }>(
          `${this.baseUrl}/components/relationships`
        ),
      60_000
    );
  }
  async getComponentPlugins(): Promise<ComponentPluginsResponse> {
    return request<ComponentPluginsResponse>(`${this.baseUrl}/components/plugins`);
  }
  async getPluginComponents(
    name: string
  ): Promise<{ plugin: string; components: ComponentContribution[] }> {
    return request<{ plugin: string; components: ComponentContribution[] }>(
      `${this.baseUrl}/components/plugins/${name}`
    );
  }
  async registerComponents(
    ...
    const result = await request<{ registered: string; count: number }>(
      `${this.baseUrl}/components/register`,
      { method: 'POST', body: { plugin, contributions } }
    );
  ...
  async unregisterPluginComponents(plugin: string): Promise<{ plugin: string; removed: boolean }> {
    const result = await request<{ plugin: string; removed: boolean }>(
      `${this.baseUrl}/components/plugins/${plugin}`,
      { method: 'DELETE' }
    );
    this.cache.invalidatePattern('component');
    return result;
  ...
  async clearData(targets: string[] = ['all']): Promise<{ success: boolean; cleared: string[] }> {
    const result = await request<{ success: boolean; cleared: string[] }>(
      `${this.baseUrl}/clear-data`,
  ...
  // === Export / Import API ===
  async exportData(scope: string = 'all'): Promise<unknown> {
    return request<unknown>(`${this.baseUrl}/export?scope=${scope}`);
  }
  async exportConversation(id: string): Promise<unknown> {
    return request<unknown>(`${this.baseUrl}/export/conversations/${id}`);
  }
  async importData(data: unknown): Promise<{ success: boolean; imported: string[] }> {
    return request<{ success: boolean; imported: string[] }>(`${this.baseUrl}/import`, {
      method: 'POST',
      body: data,
    });
  }
  ...
  async downloadResource(type: DownloadResourceType, id: string): Promise<Blob> {
    return request<Blob>(
      `${this.baseUrl}/download/resource?type=${type}&id=${encodeURIComponent(id)}`
    );
  }
  ...
  async getServerDownloadStatus(taskId: string): Promise<DownloadTask> {
    return request<DownloadTask>(
      `${this.baseUrl}/download/server/${encodeURIComponent(taskId)}/status`
    );
  }
  async controlServerDownload(
    taskId: string,
    action: 'pause' | 'resume' | 'cancel'
  ): Promise<DownloadTask> {
    return request<DownloadTask>(
      `${this.baseUrl}/download/server/${encodeURIComponent(taskId)}/${action}`,
      { method: 'POST' }
    );
  }
}
