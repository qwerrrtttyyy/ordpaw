...
export function setupApiRoutes(app: Application) {
  ...
  router.post(
    '/agents',
    validateBody<{ name: string }>({ name: 'string' }),
  ...
  router.post(
    '/providers',
    validateBody<{ name: string; type: string }>({ name: 'string', type: 'string' }),
    asyncHandler(async (req: Request, res: Response) => {
  ...
  router.post(
    '/conversations',
    validateBody<{ agentId: string }>({ agentId: 'string' }),
    asyncHandler(async (req: Request, res: Response) => {
  ...
  router.post(
    '/conversations/:id/rollback/:checkpointId',
    asyncHandler(async (req: Request, res: Response) => {
  ...
  router.post(
    '/test-suites',
    validateBody<{ agentId: string; name: string }>({ agentId: 'string', name: 'string' }),
    asyncHandler(async (req: Request, res: Response) => {
  ...
  router.post(
    '/test-suites/:id/cases',
    validateBody<{ name: string; input: string }>({ name: 'string', input: 'string' }),
    asyncHandler(async (req: Request, res: Response) => {
  ...
  router.post(
    '/chat',
    validateBody<{ conversationId: string; content: string }>({
  ...
  router.post(
    '/prompts',
    validateBody<{ name: string; content: string }>({
      name: 'string',
      content: 'string',
    }),
    asyncHandler(async (req: Request, res: Response) => {
  ...
  router.post(
    '/plugins/install',
    validateBody<{ name: string; manifest: Record<string, unknown> }>({
      name: 'string',
      manifest: 'object',
    }),
    asyncHandler(async (req: Request, res: Response) => {
  ...
  router.put(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      db.run('UPDATE plugins SET config_json = ? WHERE id = ?', [
  ...
  router.put(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      for (const [key, value] of Object.entries(settings)) {
        ...
        db.run('INSERT OR REPLACE INTO settings (key, value_json) VALUES (?, ?)', [
          key,
  ...
  router.post(
    '/scripts',
    validateBody<{ name: string; code: string }>({
      name: 'string',
      code: 'string',
    }),
    asyncHandler(async (req: Request, res: Response) => {
  ...
  router.post(
    '/mcp/scripts/call',
    validateBody<{ tool: string; params: Record<string, unknown> }>({
      tool: 'string',
      params: 'object',
    }),
    asyncHandler(async (req: Request, res: Response) => {
  ...
  router.get(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename="agent-studio-export-${Date.now()}.json"`
      );
      res.json(data);
    })
  );
  ...
  router.get(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      const data = {
        ...
        scope: 'conversation',
        conversation: conv,
        messages: queryAllLocal('SELECT * FROM messages WHERE conversation_id = ?', [
          req.params.id,
        ]),
        checkpoints: queryAllLocal('SELECT * FROM checkpoints WHERE conversation_id = ?', [
          req.params.id,
        ]),
      };
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename="conversation-${req.params.id}.json"`
      );
      res.json(data);
    })
  );
  ...
  router.post(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (data.agents)
        insertIf('agents', data.agents as unknown[], [
          'id',
          'name',
          'description',
          'system_prompt',
      ...
      if (data.providers)
        insertIf('providers', data.providers as unknown[], [
          'id',
          'name',
          'type',
          'base_url',
      ...
      if (data.conversations)
        insertIf('conversations', data.conversations as unknown[], [
          'id',
          'agent_id',
          'title',
          'variables_json',
      ...
      if (data.messages)
        insertIf('messages', data.messages as unknown[], [
          'id',
          'conversation_id',
          'role',
          'content',
      ...
      if (data.checkpoints)
        insertIf('checkpoints', data.checkpoints as unknown[], [
          'id',
          'conversation_id',
          'message_id',
          'state_json',
      ...
      if (data.prompts)
        insertIf('prompts', data.prompts as unknown[], [
          'id',
          'name',
          'category',
          'content',
      ...
      if (data.scripts)
        insertIf('scripts', data.scripts as unknown[], [
          'id',
          'name',
          'description',
          'code',
      ...
      if (data.testSuites)
        insertIf('test_suites', data.testSuites as unknown[], [
          'id',
          'agent_id',
          'name',
          'description',
      ...
      if (data.testCases)
        insertIf('test_cases', data.testCases as unknown[], [
          'id',
          'suite_id',
          'name',
          'input',
      ...
      if (data.testRuns)
        insertIf('test_runs', data.testRuns as unknown[], [
          'id',
          'suite_id',
          'agent_id',
          'results_json',
      ...
      if (data.plugins)
        insertIf('plugins', data.plugins as unknown[], [
          'id',
          'name',
          'version',
          'description',
      ...
      if (data.mcpServers)
        insertIf('mcp_servers', data.mcpServers as unknown[], [
          'id',
          'name',
          'transport',
          'command',
      ...
      if (data.installedSkills)
        insertIf('installed_skills', data.installedSkills as unknown[], [
          'id',
          'name',
          'description',
          'parameters_json',
