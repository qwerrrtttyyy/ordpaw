...
import type { DownloadItem, DownloadTask, DownloadResourceType, StorageQuota } from '@ordpaw/shared';
...
export function setupDownloadRoutes(router: Router) {
  // 单个资源下载（JSON 元数据/内容）
  router.get('/download/resource', asyncHandler(async (req: Request, res: Response) => {
  ...
  // OrdPaw 源码打包下载
  router.get('/download/source', asyncHandler(async (_req: Request, res: Response) => {
  ...
  router.get('/download/server/:id/status', asyncHandler(async (req: Request, res: Response) => {
    const record = serverTasks.get(req.params.id);
    if (!record) throw ApiError.notFound('下载任务不存在');
    res.json(record.task);
  }));

  router.post('/download/server/:id/pause', asyncHandler(async (req: Request, res: Response) => {
    const record = serverTasks.get(req.params.id);
    if (!record) throw ApiError.notFound('下载任务不存在');
  ...
  router.post('/download/server/:id/resume', asyncHandler(async (req: Request, res: Response) => {
    const record = serverTasks.get(req.params.id);
    if (!record) throw ApiError.notFound('下载任务不存在');
  ...
  router.post('/download/server/:id/cancel', asyncHandler(async (req: Request, res: Response) => {
    const record = serverTasks.get(req.params.id);
    if (!record) throw ApiError.notFound('下载任务不存在');
...
async function processServerTask(task: DownloadTask): Promise<void> {
  ...
  const quota: StorageQuota = task.storageQuota || { enforce: true, serverMaxBytes: DEFAULT_SERVER_MAX_BYTES };
    ...
    try {
      ...
      if (quota.enforce) {
        ...
        if (currentSize + contentBuffer.length > maxBytes) {
          throw new Error(`服务端存储配额不足（当前 ${currentSize}，需 ${contentBuffer.length}，上限 ${maxBytes}）`);
        }
      }
...
async function getResourcePayload(type: DownloadResourceType, id?: string): Promise<ResourcePayload> {
  ...
  switch (type) {
    ...
    case 'code': {
      ...
      return { filename: `${sanitizeFileName(script.name)}.json`, content: JSON.stringify(script, null, 2) };
    }
    ...
    case 'skill': {
      ...
      return { filename: `${sanitizeFileName(skill.name)}-skill.json`, content: JSON.stringify(serializable, null, 2) };
    }
    ...
    case 'file': {
      ...
      if (!existsSync(filePath)) {
        ...
        return { filename: `file-${id}.json`, content: JSON.stringify({ id, note: '文件未在服务器上找到' }, null, 2) };
      }
      // 二进制文件不通过 JSON 下载；这里仅返回元数据
      const stats = statSync(filePath);
      return { filename: `file-${id}.json`, content: JSON.stringify({ id, path: filePath, size: stats.size }, null, 2) };
    }
    ...
    case 'source': {
      ...
      return { filename: 'ordpaw-source.tar.gz.json', content: JSON.stringify({ path: outPath, note: '源码包已生成' }, null, 2) };
    }
